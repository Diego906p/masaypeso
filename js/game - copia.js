  ...
  scr().innerHTML = `
    <div class="welcome-wrap">
      ...
      <div class="profile-card" id="prof-papa">
        <div class="profile-emoji papa">🔐</div>
...
function isUnlocked(i){
  return isEnabled(i) && i<=P.maxPhaseReached;
}
...
function headerHTML(opts){
  ...
  if(opts.showProgress!==false){
    ...
    prog=`<div class="progress-row">
      <div class="nivel-tag">Nivel ${opts.level||1}</div>
  ...
  return `<div class="header">
    ...
    <div class="header-right">
      ...
      <div class="gear-btn" id="gear-btn">⚙️</div>
    </div>
  </div>${prog}`;
}
function bindHeader(){
  const g=$("#gear-btn");
  if(g) g.onclick=async()=>{ await flushAbandonedIfPending(); renderPhaseSelect(); };
}
...
function renderPhaseSelect(){
  ...
  scr().innerHTML=`
    ${headerHTML({phase:"Elige una fase",showProgress:false})}
    <button class="back-btn" id="back-welcome">‹ Salir</button>
    <div class="phase-grid">${cards}</div>`;
  bindHeader();
  $("#back-welcome").onclick=logout;
...
function renderPlay(){
  runtime.ex=generateExercise(P.currentPhase);
  runtime.answered=false;
  runtime.hintOn=false;
  scr().innerHTML=`
    ${headerHTML({level:P.currentLevel,qIndex:P.currentQuestion})}
...
function renderBars(revealed){
  ...
  const rows=ex.parts.map(p=>{
    ...
    const pct=show?Math.round((p.value/ex.totalValue)*100):0;
    return `<div class="bar-row">
      <div class="bar-ico ${IC_BG[p.col]}">${icon(p.icon,18,p.col)}</div>
      <div class="bar-lbl">${p.name}</div>
      ${show
        ?`<div class="track"><div class="fill ${FILL_C[p.col]}" style="width:${pct}%"></div></div><div class="bar-val ${BV_C[p.col]}">${fmtPlain(p.value)}</div>`
  ...
  const totalRow=`<div class="bar-row">
    ...
    ${totalShow
      ?`<div class="track"><div class="fill ${FILL_C.t}" style="width:100%"></div></div><div class="bar-val ${BV_C.t}">${fmtPlain(ex.totalValue)}</div>`
...
async function pick(i){
  ...
  renderBars(true);
  const fb=$("#feedback");
  fb.className="feedback visible "+(correct?"ok":"bad");
  fb.innerHTML=(correct?"✅ ¡Correcto! ":"❌ Casi... ")+ex.explain;
  if(correct){ P.stars++; P.streak++; if(P.streak>P.bestStreak) P.bestStreak=P.streak; runtime.phaseCorrect++; }
  else{ P.streak=0; runtime.phaseWrong++; }
  $("#pista-btn").disabled=true;
  $("#sig-btn").classList.add("ready");
...
async function nextQuestion(){
  P.currentQuestion++;
  if(P.currentQuestion>=QUESTIONS_PER_LEVEL){
    P.currentQuestion=0; P.currentLevel++;
    if(P.currentLevel>LEVELS_PER_PHASE){ await finishPhase(); return; }
  }
  await Store.saveProgress(P);
  renderPlay();
}
...
function renderEndPhase(s){
  const hasNext=P.currentPhase<PHASE_TOTAL-1;
  scr().innerHTML=`
    <div class="end-screen">
      ...
      <div class="end-stats">
        <div class="end-stat"><b>${s.correct}</b><span>Correctas</span></div>
        <div class="end-stat"><b>${s.wrong}</b><span>Incorrectas</span></div>
        <div class="end-stat"><b>${s.unanswered}</b><span>Sin responder</span></div>
      </div>
      ...
      <div class="end-actions">
        ${hasNext?`<button class="continue-btn" id="end-continue">Continuar ›</button>`:""}
        <button class="exit-btn" id="end-exit">Salir</button>
      </div>
    </div>`;
  $("#end-exit").onclick=()=>renderPhaseSelect();
  const c=$("#end-continue");
  if(c) c.onclick=()=>enterPhase(P.currentPhase+1);
}
...
function bindLifecycle(){
  document.addEventListener("visibilitychange",()=>{ if(document.visibilityState==="hidden") flushAbandonedIfPending(); });
  window.addEventListener("beforeunload",()=>{ flushAbandonedIfPending(); });
}

document.addEventListener("DOMContentLoaded",()=>{ if(document.getElementById("screen")) renderWelcome(); });
